  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 503 && (errorText.includes('<html') || errorText.includes('<!DOCTYPE'))) {
         return { 
           isWafBlocked: true, 
           errorMsg: "Kết nối thất bại (Lỗi 503 WAF): KiotViet hiện chặn truy cập từ môi trường Cloud (AI Studio). Mẹo: Hãy sử dụng nút 'Export', tải mã nguồn về và chạy trên máy tính nội bộ (Local) của bạn để lấy dữ liệu thuật từ KiotViet."
         };
      }
